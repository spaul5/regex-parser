...
public class NFA {
	...
	public static class State {
		...
		public State() { stateNumber = globalStateCounter++; }
		...
		public void accept(int alt) { accept = alt; }
		public String toString() { return "s"+stateNumber; }
	}
	...
	public static NFA error() { return atom('\uFFFF'); }
	...
	protected void toDOT_(State p, StringBuilder buf, Set<Integer> visited) {
		...
		for (NFA.Edge e : p.edges) {
			buf.append("  ").append(p.stateNumber).append(" -> ").append(e.target.stateNumber).append(" [label=\"");
			if (e.label == EPSILON) {
				buf.append("&epsilon;");
			}
			else {
